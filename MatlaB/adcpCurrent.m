function [hcsm,cdm,ncsm,ecsm,vcsm,binDepth,date,pitch,roll]=adcpCurrent(adcpFile)
% %input: 
...
%read binDepth data
binDepth    =   ncreadatt(adcpFile,'/','instrument_nominal_depth')- ncread(adcpFile, 'HEIGHT_ABOVE_SENSOR');
%top to bottom
...
%read ncsm and Qc data (Northward velocity)
ncsm    =   ncread(adcpFile, 'VCUR');
ncsmQc  =   ncread(adcpFile, 'VCUR_quality_control');
ncsm(ncsmQc~=1) =   NaN;
ncsm    =   squeeze(ncsm);
ncsm    =   flip(ncsm);

%read ncsm and Qc data (Eastward velocity)
ecsm    =   ncread(adcpFile, 'UCUR');
ecsmQc  =   ncread(adcpFile, 'UCUR_quality_control');
ecsm(ecsmQc~=1) =   NaN;
ecsm    =   squeeze(ecsm);
ecsm    =   flip(ecsm);
